...
void read_Raw(String path) {
  ...
  send_raw(signal_to_send, signalLenght);
...
void send_raw(long signal_send[],int Len)
{int khz = 38;
 int converted[Len];
/******************************************CONVERSION IN TO THE OTHER VECTOR**********************************/
  for(int conv=0;conv < Len;conv++)
  converted[conv] = (int)signal_send[conv];
/************************************************************************************************************/
  for(int l=0;l<20;l++)
  {
    irsend.sendRaw(converted, Len, khz); 
    delay(200);
  }
