...
{  
  ...
  else
  {
    ...
    char *aux_activity = zero_padd(activity, 2);
    Serial.println("THIS IS AUX ACT");
    Serial.println(aux_activity);
    
    char *aux_extra_value = zero_padd(extra_value, 4);
    Serial.println("THIS IS AUX EXTRA");
    Serial.println(aux_extra_value);
    
    char *aux_extra_value2 = zero_padd(extra_value2, 6);
    Serial.println("THIS IS AUX EXTRA 2");
    Serial.println(aux_extra_value2);
    
    int n = sprintf(result, "[%s,%s,%s] ", aux_activity, aux_extra_value, aux_extra_value2 );
    bluetooth.print(result);
    
    Serial.print("The previous string is long :");
    Serial.println(n);
...
char* zero_padd(int string, int spaces)
{char *aux = malloc(spaces+1);
 char *z="";
 char *res_padding="";

 
 itoa ( string, aux, 10 );
 Serial.println("This is aux :");
 Serial.print(aux);
 Serial.println(" -End of aux.");
 Serial.println("Lenght of aux :");
 Serial.print(strlen(aux));
 Serial.println(" -End");
 int padders = spaces-strlen(aux);
 Serial.print("This is padders :");
 Serial.println(padders);

 Serial.println("FlagBLB ");
 if (padders!=0)
 {
  if (padders==1)
    z="0";
  ...
  snprintf(res_padding,"%s%s",z,aux);
  Serial.println("FlagBLA ");
