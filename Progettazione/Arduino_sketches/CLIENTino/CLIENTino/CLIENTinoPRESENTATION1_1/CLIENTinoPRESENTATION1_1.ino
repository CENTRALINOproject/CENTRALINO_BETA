...
/***************************PLEASE MODIFY THIS LINE WHEN NEEDED*************************/ #define total_relay 2    // Total number of relay connected
...
bool ir_handler(decode_results *results)
{
  ...
  if (results->value == 0x1EE1F906 )
    turn_off_pin(rel[0].socket);
    else
  if (results->value == 0x1EE1C03F )
    turn_on_pin(rel[0].socket);
    else
  if (results->value == 0x1EE140BF )
    turn_off_pin(rel[1].socket);
    else
  if (results->value == 0x1EE120DF )
    turn_on_pin(rel[1].socket);
